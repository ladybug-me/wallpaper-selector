...
fn log_diag() {
    ...
    let socket = std::env::var("SKWD_DAEMON_SOCK")
    ...
    for key in ["WAYLAND_DISPLAY", "XDG_RUNTIME_DIR", "SKWD_WALL_NEXT_CONFIG", "SKWD_DAEMON_SOCK"] {
...
fn main() {
    ...
    if args.help {
        println!("skwd-wall {}", env!("CARGO_PKG_VERSION"));
        println!("usage: skwd-wall [--debug] [--version | -V]  (no arguments launches the picker)");
